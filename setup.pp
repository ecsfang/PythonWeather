...
#     '--no-install-recommends',
#   ],
#   before          => Python::Requirements['/home/pi/PythonWeather/requirements.txt'],
# }
...
vcsrepo { '/home/pi/PythonWeather':
  ensure   => latest,
  provider => git,
  source   => 'https://github.com/genebean/PythonWeather.git',
}
...
systemd::unit_file { 'PythonWeather.service':
  source  => 'file:///home/pi/PythonWeather/PythonWeather.service',
  require => Python::Pip[$python_packages],
  notify  => Service['PythonWeather.service'],
}

service {'PythonWeather.service':
  ensure    => running,
  enable    => true,
  require   => Systemd::Unit_file['PythonWeather.service'],
  subscribe => [
    Exec['enable display-setup-script'],
